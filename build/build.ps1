...
$Spec = if ($Debug) { "build\LogNotes.debug.spec" } else { "build\LogNotes.spec" }
...
if ($Debug) {
    Write-Host "==> Debug build complete: dist/LogNotes-debug/LogNotes-debug.exe" -ForegroundColor Green
    exit 0
}

Write-Host "==> Build complete: dist/LogNotes/LogNotes.exe" -ForegroundColor Green
